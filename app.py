...
def plot_deviation_trend_chart(key: str) -> go.Figure:
    ...
    df = pd.DataFrame(data); fig = px.area(df, x='Month', y=['Bioreactor Suite C', 'Purification Skid A', 'Packaging Line 2'], title='<b>Deviation Trend by Manufacturing System</b>', markers=True)
    fig.update_layout(yaxis_title="Number of Deviations", title_x=0.5, plot_bgcolor=BACKGROUND_GREY, legend_title_text='System')
    return fig
...
def render_strategic_management_page() -> None:
    ...
    with col1:
        with st.container(border=True):
            st.subheader("Annual Budget Performance", help="Tracks actual spend against the department's annual budget.")
...
def render_project_portfolio_page() -> None:
    ...
    with col1:
        with st.container(border=True):
            st.subheader("Validation Team Resource Allocation")
            ...
            if not over_allocated_df.empty:
                for _, row in over_allocated_df.iterrows():
                    st.warning(f"**⚠️ Over-allocation Alert:** {row['Team Member']} at {row['Total Allocation']:.0%}.")
    with col2:
        with st.container(border=True):
            st.subheader("Key Vendor Performance Scorecard")
            display_vendor_scorecard("vendor")
            st.success("**Actionable Insight:** Vendor A's strong performance makes them a preferred partner. Vendor B's performance requires a follow-up meeting to establish a performance improvement plan.")
...
def render_validation_program_health_page() -> None:
    ...
    with tab2:
        ...
        st.success("**Actionable Insight:** The rising deviation trend in the Bioreactor Suite C directly validates the focus of our Kaizen efforts. The ROI tracker provides a strong business case for these improvement projects.")
...
def render_documentation_hub_page() -> None:
    ...
    with col1:
        with st.container(border=True):
            st.subheader("Document Approval Workflow"); st.info("Simulates the eQMS workflow."); st.markdown("Status for `VAL-MP-001_Project_Atlas`:"); st.divider()
